...
while(1):
	...
	if inst=='01':
		...
		rn=int(omemory[k][12:16],16)
		rd=int(omemory[k][16:20],16)
		offset=int(omemory[k][20:32],16)
		if strldr=='11000':
			result=(hex(res[rd]).lstrip('0x')).zfill(8)
			print result
		#	res[rn]+offset	
			memory.append((accu,result[6:8]))
			accu+=1
			memory.append((accu,result[4:6]))
			accu+=1
			memory.append((accu,result[2:4]))
			accu+=1
			memory.append((accu,result[0:2]))
			accu+=1
